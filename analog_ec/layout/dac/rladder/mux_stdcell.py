...
class PassgateRow(StdCellBase):
    ...
    **kwargs :
    ...
    def draw_layout(self):
        ...
        # add pass gates.
        xo = 0
        pg_inst = self.add_std_instance(pg_master, loc=(xo, 0), nx=num_col + 1,
                                        spx=pg_master.std_size[0])
        ...
        s_tid = s_tid.transform(self.grid, pg_inst.location_unit, pg_inst.orientation,
                                unit_mode=True)
...
class InputBuffer(StdCellBase):
    ...
    **kwargs :
    ...
    def draw_layout(self):
        ...
        top_inst = self.add_std_instance(inv_master, loc=(0, 1), nx=num_bits,
                                         spx=inv_master.std_size[0])
    ...
    **kwargs :
    ...
    def draw_layout(self):
        ...
        for idx, (pin_list, outb_idx_list, out_idx_list) in enumerate(in_lists):
            outb_tr = self.grid.find_next_track(in_layer, pin_list[0].lower, mode=1,
                                                half_track=True)
...
class ColDecoder(StdCellBase):
    ...
    **kwargs :
    ...
    def draw_layout(self):
        ...
        in_layer = in_lists[0][0][0].layer_id + 1
        for idx, (pin_list, outb_idx_list, out_idx_list) in enumerate(in_lists):
            outb_tr = self.grid.find_next_track(in_layer, pin_list[0].lower, half_track=True,
                                                mode=1)
            warr = self.connect_to_tracks([pin_list[idx] for idx in outb_idx_list],
                                          TrackID(in_layer, outb_tr))
            self.add_pin('inb<%d>' % idx, warr, show=show_pins)
...
class RLadderMux(StdCellBase):
    ...
    **kwargs :
    ...
    def draw_layout(self):
        ...
        rdec_inst = self.add_std_instance(row_master, loc=(xoff + rdec_col_idx, row_offset),
                                          flip_lr=True)
        ...
        for idx in range(num_row):
            ...
            for cidx in range(num_col):
                for cpidx, pfmt in enumerate(('en<%d>', 'enb<%d>')):
                    col_port_list[cidx][cpidx].append(
                        pg_inst.get_port(pfmt % cidx, row=ridx).get_pins()[0])
        ...
        for idx in range(num_col):
            ...
            self.connect_to_tracks(col_port_list[idx][0], col_out.track_id,
                                   track_lower=col_out.lower)
            self.connect_to_tracks(col_port_list[idx][1], col_outb.track_id,
                                   track_lower=col_outb.lower)
    ...
    def _up_two_layers(self, warr, mid_tr):
        ...
        warr = self.connect_to_tracks(warr, TrackID(lay_id, mid_tr),
        ...
        min_len = self.grid.get_min_length(lay_id, 1)
        return self.connect_to_tracks(warr, TrackID(lay_id, warr_tr),
                                      track_lower=mid_coord - min_len / 2,
                                      track_upper=mid_coord + min_len / 2)
